...
public class GravityPanel extends JPanel implements MouseListener, ActionListener, KeyListener{
	...
	public void updateFollowMouseMode() {
		...
		try {
			...
			System.out.println("Moose not found!!!");
		}
		object.update();
	...
	public void draw(Graphics g) {
		...
		switch(state) {
		case FOLLOW_MOUSE:
			Gravity_Circle_Demo.getFrame().setTitle("Follow Mouse Mode");
			break;
		case GRAVITY_MODE:
			Gravity_Circle_Demo.getFrame().setTitle("Gravity Mode");
			break;
		case PAUSE_MODE:
			Gravity_Circle_Demo.getFrame().setTitle("Paused");
			break;
		case DRIFT_MODE:
			Gravity_Circle_Demo.getFrame().setTitle("Drift Mode");
			break;
		default:
			break;
		}
	...
	public void mouseClicked(MouseEvent e) {
		...
		if(state!=State.FOLLOW_MOUSE&&clickState==ClickState.PINNED_MASS) {
			if(isMouseVisible()) {
				if(buttonClicked == 1) {
					manager.addPin((int)getPosition().getX(), (int)getPosition().getY(), Gravity_Circle_Demo.getMassValueOfControlPanel());
				}
				if(buttonClicked == 3) {
